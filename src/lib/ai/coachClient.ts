...
/**
 * Calls the deployed AI Edge Function (see supabase/functions/ai-coach) with
 * `type: "coach"` when a project is configured; otherwise (or on any failure)
 * falls back to the deterministic local responder so the chat still works
 * fully without a live backend.
 */
  ...
  if (isSupabaseConfigured) {
    try {
      ...
      const { data, error } = await supabase.functions.invoke(AI_FUNCTION_NAME, {
        body: { type: "coach", conversationId, message: userText, history: history.slice(-20), context },
      });
