...
export function Stepper({
  ...
  return (
    <div className="flex flex-col items-center">
      ...
      <div className="flex items-center gap-1.5 bg-surface border border-line-subtle rounded-xl p-1">
        <button
          type="button"
          onClick={() => onChange(Math.max(min, Math.round((value - step) * 100) / 100))}
          className="w-11 h-11 flex items-center justify-center rounded-lg hover:bg-surface-hover active:bg-surface-hover text-ink-dim"
        >
          <Minus size={16} />
        </button>
